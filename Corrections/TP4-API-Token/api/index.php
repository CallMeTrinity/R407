...
trait Controller {
    ...
    private function validateToken($token) {
        $req = "SELECT * FROM TOKEN WHERE token = '{$token}'";
        return sizeof($this->api->select($req)) == 1;
    }
...
class Preparation {
    ...
    public function list() {
        ...
        $this->res['data'] = $this->api->list(<<<EOL
                    ...
                    WHERE recipe = {$this->data['rid']} AND
                          recipe = Recipe.id AND ingredient = Ingredient.id
            EOL);
        $this->res['msg'] = "List all ingredients of recipe {$this->data['rid']}.";
        $this->stop(true);
    }
...
class User {
    ...
    public function getUser($login) {
        ...
        return $this->api->select($req);
    }

    public function setUserToken($login, $token) {
        $req = "INSERT INTO Token (login, token) VALUES ('{$login}', '{$token}')";
        $this->api->instruct($req);
    }

    public function unsetUserTokens() {
        $req = "DELETE FROM Token";
        $this->api->instruct($req);
    }
